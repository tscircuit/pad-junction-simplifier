...
import type { HighDensityRoute } from "./routeTypes"
import { getGraphicsLayerForObstacle } from "./getGraphicsLayerForObstacle"
import type {
  Pad,
...
export function visualizePadJunctionSimplification(
  ...
  for (const route of [...hdRoutes, ...(input.otherHdRoutes ?? [])]) {
    ...
    for (let index = 1; index < route.route.length; index++) {
      ...
      let strokeDash: Line["strokeDash"]
      if (start.z !== 0) strokeDash = [0.08, 0.08]
      lines.push({
        points: [start, end],
